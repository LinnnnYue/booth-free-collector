...
- **分类汉化**：`booth_common.CATEGORY_MAP`（R12 同步：含中日表示 `3D髪型→3D发型`、`3D衣装→3D服饰`、`3D小道具→3D道具`；`ソフトウェア→软件`；`素材（その他）→素材数据`；未知类目保留日文原名不臆造）。**与 BoothKeeper 桌面版 `booth_core.CATEGORY_MAP` 保持同源同步**——两边归档目录必须一致，否则互相乱放对面不认识的位置
- **父级 3D 前缀规则**：`classify()` 对父类是 `3Dモデル`/`3Dモデル（その他）`/`3D衣装・アクセサリー`/`アバター` 的子分类，若映射结果不带「3D」前缀自动补全（如 `衣装 + 3Dモデル → 3D服饰`）——避免新建「服饰」「发型」等无前缀目录
- **商品页下载文件名 = 终极锚点**：候选歧义时 `fetch_item_downloads(item_id, session)` 解析商品页 HTML 下载区（`<a title="filename.zip" href="downloadables/{fid}?variation_id=...">`），直接拿真实文件名（如 3562410 → `メカ弾エフェクトVer_2.00.zip`），与本地文件前缀/版本号匹配 100% 确认（R9 同步：JSON API `files` 字段为空，必须解析 HTML）
