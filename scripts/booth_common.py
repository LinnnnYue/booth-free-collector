...
# 注意：与 BoothKeeper 桌面版 booth_core.py 保持同步（R12/R12+1 三次增量已合并）
CATEGORY_MAP = {
    # 3D / VRChat 头像系
    ...
    "衣装": "服饰",
    # R12 同步：BOOTH 用「中日表示」cat_name（'3D髪型' '3D衣装' 等），漏映射会 fallback 父级误判
    "髪": "3D发型", "ヘアー": "3D发型", "ヘア": "3D发型",
    "3D髪型": "3D发型",      # 3D Hair
    "3D衣装": "3D服饰",      # 3D Clothing
    "3D小道具": "3D道具",     # 3D Prop
    ...
    "物理": "物理", "VR": "VR",
    "ソフトウェア": "软件", "ソフトウェア・ハードウェア": "软件",  # R12+1：补父级
    ...
    "素材データ": "素材数据", "音楽": "音乐", "アバター": "头像",  # R12+1：去重，统一头像
    "素材（その他）": "素材数据",  # R12+1：その他=其他，归入素材数据
    "アクセサリー": "配饰",
}
CATEGORY_PARENT_MAP = {"3Dモデル": "3D模型", "ゲーム": "游戏", "アバター": "头像"}

# R12 同步：父类属于以下时，子分类强制加 3D 前缀（与 BoothKeeper 一致）
...
def classify(cat_name: str, cat_parent: str = "") -> str:
    ...
    R12 同步：父级 3D 前缀规则——父类是 3D 系（3Dモデル/アバター），
    但 cat_name 映射结果不带「3D」前缀时自动补，让子分类归入 3D 前缀目录结构。
    """
    if not cat_name:
        return "未分类"
...
def fetch_item(item_id: str, session: requests.Session | None = None) -> dict | None:
    """拉 BOOTH JSON API。失败返回 None。

    注意：返回 raw JSON（调用方 booth.py 依赖 `item.get("category").get("name")`），
    如需规范化 schema 用 _normalize_item()，分类用 classify_item()。
    """
...
def _normalize_item(d: dict | None) -> dict | None:
    """把 BOOTH JSON API 原始 dict 拍扁成统一 schema（与 BoothKeeper booth_core 同步）。

    关键字段：category_name / category_parent_name（raw JSON 是 category.name / category.parent.name）
    历史坑：调用方用 classify(it.get("category_name"), it.get("category_parent_name"))
    在 raw JSON 上全 None → 归「未分类」。统一拍扁后调用方一致可用。
    """
    if not d:
        return None
...
def classify_item(item: dict) -> str:
    """从 fetch_item 结果（raw 或 normalized 均可）取类目并 classify。"""
    if not item:
        return "未分类"
    ...
    return classify(cat_name, cat_parent)


def fetch_item_downloads(item_id: str, session: requests.Session | None = None) -> list[dict]:
    """从 BOOTH 商品页面 HTML 解析所有下载链接 + 文件名 + 大小（与 BoothKeeper booth_core 同步）。
    ...
    BOOTH 下载按钮结构（实测 2026-08-16）：
      <a class="btn add-cart" title="RuinHalo_v1.01.zip"
        ...
        文件名在 title 属性；大小在 <div>(1.82 MB)</div>
    """
    s = session or make_session()
